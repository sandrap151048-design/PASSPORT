...
export default function AdminServices() {
  ...
  const fetchServices = async () => {
    try {
      ...
      alert('Error loading services. Please check if the server is running.');
    }
  };
  ...
  const resetForm = () => {
    setFormData({ title: '', description: '', imageUrl: '', features: [], isActive: true });
    setEditingId(null);
    setShowForm(false);
  };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8 animate-slide-in-left">
          <div>
            ...
            <p className="text-gray-600 mt-1">Manage services offered to students</p>
          </div>
          <button
          ...
          <div className="bg-white p-8 rounded-2xl shadow-xl mb-8 animate-zoom-in border border-gray-100">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <input type="text" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all" placeholder="e.g., Study Abroad Consulting" required />
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Description *</label>
                <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all" rows="4" placeholder="Describe the service..." required></textarea>
              </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {services.map((service, index) => (
                  <tr key={service._id} className="hover:bg-gray-50 transition-colors animate-fade-in" style={{ animationDelay: `${index * 50}ms` }}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-3 py-1 text-xs font-semibold rounded-full ${service.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                        {service.isActive ? '✓ Active' : '✕ Inactive'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <button onClick={() => handleEdit(service)} className="text-blue-600 hover:text-blue-900 font-semibold mr-4 transition-colors inline-flex items-center gap-1">
                        ✏️ Edit
                      </button>
                      <button onClick={() => handleDelete(service._id)} className="text-red-600 hover:text-red-900 font-semibold transition-colors inline-flex items-center gap-1">
                        🗑️ Delete
                      </button>
                    </td>
                  </tr>
